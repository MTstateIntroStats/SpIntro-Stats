...
In Unit 1 we used spinners and coin flips to simulate the distribution
of a proportion.  From that distribution we used the standard
deviation as printed in the plot window.  We added two standard errors
...
continuous variable, so we'll come back to this.  For now, know this:
  What we called ``std.dev.'' in web applets is actually the ``SE'' of
...
\url{http://shiny.math.montana.edu/prob} and put
confidence levels 0.80, 0.90, $\ldots$, 0.99 into the probability box
(one at a time).
Leave the distribution set to \fbox{Normal} and change \fbox{Lower} to
...
\end{key}
     ...
     \item If we use StatKey to create a 99\% bootstrap percentile
       confidence interval, it is $(0.542, 0.597)$ and the st.dev. in
...
\end{key}
  ...
  Using normal distributions adds another assumption:
   ...
   \begin{enumerate}
     \setcounter{enumi}{1}
   \item In an earlier Quizork  you checked to see if a roulette wheel was
...
\end{key}
    ...
    \item On pages 43-46 of these notes you estimated the probability
...
\end{key}
  ...
  \item Put the standardized $z$ value into the web app 
    \url{http://shiny.math.montana.edu/prob} and ask for
...
\end{key}
\item Now switch to thinking of a hypothesis test using ``just
