...
Back on Day 14 (pages 65--70) we compared an energy drink with
alcohol, REDA to a control.  Our conclusion that ``change in RBANS''
went down did not allow us to say whether that was due to the alcohol
...
distribution. 
 ...
 Go to the site:\\
  \url{http://www.rossmanchance.com/ISIapplets.html}. Select \fbox{\sf
    Multiple Means} and click 
  \fbox{Stacked (Group Value)}. Under the data box click \fbox{\sf Clear}.
  Paste in the energy drink data from D2L including the top row which
  describes the two columns.  \vspace{.1in}\\
  \begin{fmpage}{.2\linewidth}
{\footnotesize
\begin{verbatim}
treatment RBAN
REDA	6.84
...
\end{key}
 ...
 \item  Go to the applet:
   \url{http://shiny.math.montana.edu/prob}. Put your t
   test statistic in the top box, select the \fbox{t} distribution,
